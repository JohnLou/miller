...
var MSocket = exports.MSocket = function (socket, options) {
    if (!(this instanceof MSocket)) {
        return new MSocket(socket, options);
    }
...
exports.createServer = function createServer(options, connectionListener) {
    ...
    function onConnection (socket) {
        ...
        connectionListener(new MSocket(socket, options));
    }
...
//
MSocket.prototype.send = function send(event, data, callback) {
    ...
    if (!this.socket || !this.connected) {
        return this.emit('error', new Error('MSocket: sending on a bad socket'));
    }
...
//
MSocket.prototype.data = function (event, callback) {
    if (typeof event === 'string') {
        event = event.split(this._delimiter);
    }
...
MSocket.prototype.undata = function (event, listener) {
...
//
MSocket.prototype.dataOnce = function (event, callback) {
    if (typeof event === 'string') {
        event = event.split(this._delimiter);
    }
...
//
MSocket.prototype.setIdle = function setIdle(time) {
...
// #### forcibly destroys this MSocket, unregister socket, remove all callbacks
//
MSocket.prototype.destroy = function destroy() {
...
//
MSocket.prototype.end = function end() {
    var hadErr;
...
//
MSocket.prototype.connect = function connect(/*port, host, callback*/) {
...
//
MSocket.prototype.reconnect = function reconnect() {
    var self = this;
...
//
MSocket.prototype._setup = function () {
    var self = this,
        startName;
...
//
MSocket.prototype._onStart = function _onStart() {
    this.emit('start');
};
...
// Messages are assumed to be delimited properly (if using MSocket to send)
// otherwise the delimiter should exist at the end of every message
// We assume messages arrive in order.
//
MSocket.prototype._onData = function _onData(message) {
    var parsed,
        data;
...
//
MSocket.prototype._onClose = function _onClose(hadError) {
    if (hadError) {
...
//
MSocket.prototype._onError = function _onError(error) {
    this.connected = false;
...
//
MSocket.prototype._onIdle = function _onIdle() {
    this.emit('idle');
    if (this._timeout) {
